...
export async function getPipeline(type: string, model: string, options: any = {}) {
  const lib = await ensureHF();
  const backend = await detectBackend();
  const device = backend === 'webgpu' ? 'webgpu' : 'wasm';
  const pipeline = await lib.pipeline(type as any, model, { device, ...options });
  (window as any).electronAPI?.llm?.update?.({ backend, updatedAt: Date.now() });
