...
function parseDigestChallenge(wwwAuthenticate) {
  const challenge = {};
  const parts = wwwAuthenticate.replace(/Digest\s+/, '').split(',');
 ...
 */
function generateDigestResponse(username, password, method, uri, challenge) {
  ...
  // Generate nc (nonce count) - for simplicity, we'll use 00000001
  const nc = '00000001';
 ...
 */
export async function digestAuthRequest(url, options = {}) {
  const { username, password, ...fetchOptions } = options;
  
  if (!username || !password) {
    throw new Error('Username and password are required for digest authentication');
  }
  ...
  // Check if we got a 401 with WWW-Authenticate header
  if (firstResponse.status === 401) {
    ...
    if (wwwAuthenticate && wwwAuthenticate.startsWith('Digest')) {
      ...
      // Generate digest response
      const authHeader = generateDigestResponse(
        username, 
        password, 
  ...
  // If no digest challenge, return the original response
